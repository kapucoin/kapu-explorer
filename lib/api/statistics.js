...
module.exports = function (app) {
    ...
    this.getBlocks = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('ark address') + blocks.url(offset, limit),
                    json : true
    ...
    this.getLastBlock = function (error, success) {
        ...
        request.get({
            url: app.get('ark address') + blocks.url(0, 1),
            json : true
        }, function (err, resp, body) {
            if (err || resp.statusCode !== 200) {
    ...
    function Peers (locator) {
        ...
        this.process = function (p, next) {
            ...
            // temporary fix for new status field
            if ('OK' === p.status) {
            ...
            // Check for duplicates because KAPU API ignore limit/offset
            // if (this.peers.indexOf (p.ip + p.port) !== -1) {
            //     // Is duplicate - skip
            //     return next();
            // } else {
            //     this.peers.push (p.ip + p.port);
            //     // Delete unused properties
            //     delete p.string;
            //     delete p.blockheader;
            // }
    ...
    this.getPeers = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('ark address') + peers.url(offset, limit),
                    json : true
                }, function (err, resp, body) {
                    if (err || resp.statusCode !== 200) {
                        return next(err || 'Response was unsuccessful');
